            ...
            if (password.length() >= 8 && containsCapitalLetter(password) && containsSpecialCharacter(password)) {
                ...
                if (email.matches(emailRegex)) {
                    try {
                        ...
                        PreparedStatement checkUsername = con.prepareStatement("SELECT * FROM admin WHERE tUname = ?");
                        ...
                        PreparedStatement ps = con.prepareStatement("INSERT INTO admin (tName, tUname, tID, tGender, tEmail, tPassword, image) VALUES (?,?,?,?,?,?,?)");
